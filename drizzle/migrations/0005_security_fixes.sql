...
-- Fixes for: (1) client-authoritative catch RNG / dupe exploit,
--             (2) missing cast cooldown, (3) signature replay window.
-- Safe to run once on top of 0000-0004 + the three supabase/migrations/*.sql.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. Cast cooldown: track the last time each wallet successfully recorded a
--    catch, and a configurable minimum interval between casts.
-- -----------------------------------------------------------------------------
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS last_cast_at timestamptz;

INSERT INTO public.game_config (key, value) VALUES
  ('cast_cooldown_seconds', 1.5)
ON CONFLICT (key) DO NOTHING;

-- -----------------------------------------------------------------------------
-- 2. Signature replay window — NOT fixed with a single-use nonce here.
--    This app signs the wallet ONCE per session and reuses that same proof
--    for every action (cast, sell, buy, equip — see useProfileStore.proof).
--    A single-use-per-call nonce would reject every action after the first
--    one and break the game. The real mitigation shipped for this is
--    shortening SIGNATURE_MAX_AGE_MS in src/lib/walletAuth.ts from 24h to
--    30 minutes, which cuts the replay exposure window ~48x without
--    changing the auth architecture. Eliminating replay completely needs a
--    proper session-token upgrade (sign once -> server issues a short-lived
--    session token -> client sends the token, not the raw signature, on
--    every subsequent call). That's a bigger architectural change than a
--    patch and isn't included here — flag it separately if you want it.
-- -----------------------------------------------------------------------------

-- -----------------------------------------------------------------------------
-- 3. record_catch is now fully server-authoritative: it no longer accepts
--    species/rarity/weight/mutation from the caller at all. It re-implements
--    the same weighted-roll rules as src/hooks/useGameStore.ts::rollFish,
--    but reads equipped rod/bait from player_rods/player_baits and rolls with
--    the server's own RNG. The old 5-arg signature is dropped.
-- -----------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.record_catch(text, text, text, numeric, text);
DROP FUNCTION IF EXISTS public.record_catch(text, text);

-- NOTE: every RETURNS TABLE column name below is implicitly declared as a
-- plpgsql variable inside the function body. They are all prefixed with
-- "out_" so none of them can ever shadow a real table column (fish_species
-- has columns literally named color/rarity/is_monster, etc. — an unprefixed
-- "is_monster" here previously caused "column reference is ambiguous").
...
DECLARE
  ...
  monster_chance numeric;
  is_monster_roll boolean := false;
...
BEGIN
  ...
  -- ---- monster roll -----------------------------------------------------
  SELECT value INTO monster_chance FROM public.game_config WHERE key = 'monster_catch_chance';
  monster_chance := coalesce(monster_chance, 0);
  IF EXISTS (SELECT 1 FROM public.fish_species WHERE is_monster = true)
     AND random() < monster_chance THEN
    is_monster_roll := true;
  END IF;

  IF is_monster_roll THEN
    SELECT id, name, color, rarity, min_weight_kg, max_weight_kg, is_monster
      INTO picked_species
      FROM public.fish_species WHERE is_monster = true LIMIT 1;
    picked_rarity := coalesce(picked_species.rarity, 'mythic');
  ELSE
    -- weighted pool over non-monster species within the rod's catch cap
    -- (column is "rw", not "w" — "w" is already the wallet-address variable
    -- above, and plpgsql would otherwise silently substitute the variable).
    -- Plain CTEs, not a temp table, so this is safe to call repeatedly
    ...
    WITH pool AS (
      ...
      WHERE s.is_monster = false AND s.min_weight_kg <= coalesce(rod_cap, 1e18)
    )
  ...
  -- effective rarity for XP/counters: monster catches always count as mythic,
  -- matching the client's previous display convention.
  IF is_monster_roll THEN
    picked_rarity := 'mythic';
  END IF;
  ...
  RETURN QUERY SELECT
    ...
    picked_weight, picked_mutation.key, picked_mutation.label, is_monster_roll,
